...
from V4Backend.API.workflow.utils import createPolygon, isPolygonExists, getPolygonId, extract_temperatures, \
    kelvin_to_celsius
...
@tool
def get_weather_ranges(weather_data):
    """use this to get weather range"""

    # get ranges from your tools
    temp_min, temp_max = get_min_max(filter_temp(weather_data.crop))
    soil_min, soil_max = get_min_max(filter_soil_moisture(weather_data.crop))
    light_min, light_max = get_min_max(filter_light_status(weather_data.crop))

    # compute actual values
    temp_val = avg(weather_data.temperature_record)
    soil_val = weather_data.soil_moisture
    light_val = avg(weather_data.uvi_values)
...
def get_temperatures(latitude: float, longitude: float):
    ...
    url = f"api.openweathermap.org/data/2.5/forecast?lat={latitude}&lon={longitude}&appid={OPENWEATHERAPI_KEY}"
